...
export class IndexPersistence {
  ...
  constructor(
    ...
    // When present (lancedb backend), BM25 routes through this on-disk blob
    // store and the sharded iii-KV path below is bypassed entirely. Vectors
    // persist externally per-row, so they are intentionally never serialized
    // on this path. Absent (memory/iii), the upstream sharded index is used.
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      if (this.vector) {
        await this.saveVectorIndex(await this.vector.serialize());
      }
    } catch (err) {
      this.logFailure(err);
    }
  ...
  async load(): Promise<{
    ...
    if (this.blobStore) {
      const d = await this.blobStore.get("data");
      return {
        bm25: d ? SearchIndex.deserialize(d) : null,
        vectorJson: null,
      };
    }
    ...
    // The fork's VectorIndex is an async-backed wrapper with no static
    // deserialize; return the raw shard-reassembled JSON and let the caller
    // hydrate it via vectorIndex.restoreFrom(). Null for lancedb (above).
    const vecData = await this.loadVectorData();
    return { bm25, vectorJson: vecData ?? null };
  }
